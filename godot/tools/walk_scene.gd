extends Node
## Main scene for: godot --headless --path godot res://tools/walk_scene.tscn
...
func _probe(gym: Node) -> void:
	...
	lines.append("GRID %d x %d step=%.2f open=%d blocked=%d reach=%d" % [nx, nz, STEP, open, hits, qarr.size()])
	...
	for s in lines:
		if s.begins_with("BLOCK"):
			blocked += 1
	lines.append("SUMMARY reach=%d / open=%d blocked_targets=%d" % [qarr.size(), open, blocked])
	var text := "\n".join(lines)
	print(text)
